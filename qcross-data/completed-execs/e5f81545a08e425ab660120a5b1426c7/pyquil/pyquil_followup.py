
from pyquil import Program, get_qc
from pyquil.gates import MEASURE
from pyquil.quil import Pragma
from pyquil.parser import parse_program

from bloqs.ext.pyquil import Gates, get_custom_get_definitions
from bloqs.ext.pyquil.utils import get_qiskit_like_output
        



circuit = Program()

qr = circuit.declare("ro", "BIT", 5)

p_566c5c = circuit.declare('p_566c5c', 'REAL')
p_d15eb0 = circuit.declare('p_d15eb0', 'REAL')

defns = get_custom_get_definitions("RYYGate", "CHGate", "XGate", "ECRGate", "ZGate", "TGate", "CSXGate", "CRZGate", "CRXGate", "SXdgGate", "SGate", "RZGate")

circuit += defns

circuit.inst(Gates.RZGate(p_566c5c, 1 ))
circuit.inst(Gates.SXdgGate( 3 ))
circuit.inst(Gates.CSXGate( 2, 0 ))
circuit.inst(Gates.ECRGate( 1, 3 ))
circuit.inst(Gates.CRXGate(2.0099472182748075, 4, 1 ))
circuit.inst(Gates.SGate( 0 ))
circuit.inst(Gates.CRZGate(1.0296448789776642, 1, 4 ))
circuit.inst(Gates.CHGate( 2, 4 ))
circuit.inst(Gates.RYYGate(p_d15eb0)( 0, 3 ))
circuit.inst(Gates.ZGate( 3 ))
circuit.inst(Gates.TGate( 4 ))
circuit.inst(Gates.CSXGate( 2, 4 ))
circuit.inst(Gates.XGate( 0 ))

circuit += MEASURE(0, qr[0])
circuit += MEASURE(2, qr[1])
circuit += MEASURE(3, qr[2])
circuit += MEASURE(1, qr[3])
circuit += MEASURE(4, qr[4])




circuit.wrap_in_numshots_loop(979)

qc = get_qc("9q-square-qvm", execution_timeout=60, compiler_timeout=60)

executable = qc.compile(circuit, protoquil=True)



params = {
    "p_566c5c": 6.163759533339787,
    "p_d15eb0": 1.6723037552953224
}

for param, value in params.items():
    executable.write_memory(region_name=param, value=value)
        


quil_out = circuit.out()
circuit = parse_program(quil_out) # new circuit


result = qc.run(executable).readout_data.get('ro')

counts = get_qiskit_like_output(result)
RESULT = counts


if __name__ == '__main__':
    from utils import display_results
    display_results( {"result": RESULT })

