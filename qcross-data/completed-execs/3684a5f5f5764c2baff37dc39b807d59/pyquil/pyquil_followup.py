
from pyquil import Program, get_qc
from pyquil.gates import MEASURE
from pyquil.quil import Pragma
from pyquil.parser import parse_program

from bloqs.ext.pyquil import Gates, get_custom_get_definitions
from bloqs.ext.pyquil.utils import get_qiskit_like_output
        



circuit = Program()

qr = circuit.declare("ro", "BIT", 8)

p_660fb5 = circuit.declare('p_660fb5', 'REAL')
p_773c06 = circuit.declare('p_773c06', 'REAL')
p_bfa20f = circuit.declare('p_bfa20f', 'REAL')
p_77ac42 = circuit.declare('p_77ac42', 'REAL')
p_a65f95 = circuit.declare('p_a65f95', 'REAL')

defns = get_custom_get_definitions("CRZGate", "CU1Gate", "CXGate", "TGate", "XGate", "CSXGate", "ECRGate", "U2Gate", "C3SXGate", "RZZGate", "RXGate", "UGate", "SXdgGate", "RYYGate", "CUGate", "RZGate", "ZGate", "CRXGate", "SdgGate", "SXGate")

circuit += defns

circuit.inst(Gates.RZGate(6.163759533339787, 4 ))
circuit.inst(Gates.ZGate( 6 ))
circuit.inst(Gates.XGate( 6 ))
circuit.inst(Gates.CRXGate(5.987304452123941, 0, 6 ))

subcircuit = Program()
subcircuit.inst(Gates.RXGate(0.7631154445516055, 3 ))
subcircuit.inst(Gates.CXGate( 3, 7 ))
circuit.inst(subcircuit)
circuit.inst(subcircuit.dagger())

circuit.inst(Gates.CRZGate(1.0296448789776642, 1, 6 ))
circuit.inst(Gates.C3SXGate( 0, 7, 6, 3 ))
circuit.inst(Gates.ZGate( 2 ))
circuit.inst(Gates.XGate( 1 ))
circuit.inst(Gates.RYYGate(p_bfa20f)( 6, 7 ))
circuit.inst(Gates.CRZGate(4.167661441102218, 1, 7 ))
circuit.inst(Gates.RZGate(p_77ac42, 1 ))
circuit.inst(Gates.SXGate( 0 ))
circuit.inst(Gates.CU1Gate(3.2142159669963557, 4, 0 ))
circuit.inst(Gates.CRXGate(5.94477504571567, 6, 4 ))
circuit.inst(Gates.RZZGate(p_660fb5)( 7, 0 ))
circuit.inst(Gates.CSXGate( 0, 2 ))
circuit.inst(Gates.ZGate( 6 ))
circuit.inst(Gates.RZGate(3.775592041307464, 4 ))
circuit.inst(Gates.CRXGate(0.7279391018916035, 0, 3 ))
circuit.inst(Gates.CUGate(p_773c06, 5.0063780207098425, 3.1562533916051736, 4.940217775579305, 6, 2 ))
circuit.inst(Gates.U2Gate(2.5163050709890156, 2.1276323672732023)( 2 ))
circuit.inst(Gates.TGate( 6 ))
circuit.inst(Gates.SdgGate( 0 ))
circuit.inst(Gates.RZZGate(p_a65f95)( 3, 4 ))
circuit.inst(Gates.TGate( 5 ))
circuit.inst(Gates.RYYGate(1.9669252191306448)( 4, 2 ))
circuit.inst(Gates.C3SXGate( 1, 3, 2, 5 ))
circuit.inst(Gates.SXdgGate( 7 ))
circuit.inst(Gates.UGate(5.080799300534071, 5.023617931957853, 2.271164628944128)( 2 ))
circuit.inst(Gates.ECRGate( 6, 2 ))

circuit += MEASURE(0, qr[0])
circuit += MEASURE(1, qr[1])
circuit += MEASURE(2, qr[2])
circuit += MEASURE(3, qr[3])
circuit += MEASURE(4, qr[4])
circuit += MEASURE(5, qr[5])
circuit += MEASURE(6, qr[6])
circuit += MEASURE(7, qr[7])




circuit.wrap_in_numshots_loop(2771)

qc = get_qc("9q-square-qvm", execution_timeout=60, compiler_timeout=60)

executable = qc.compile(circuit, protoquil=True)



params = {
    "p_660fb5": 5.1829934776392745,
    "p_773c06": 5.03147076606842,
    "p_bfa20f": 1.740253089260498,
    "p_77ac42": 4.229610589867865,
    "p_a65f95": 3.950837470808744
}

for param, value in params.items():
    executable.write_memory(region_name=param, value=value)
        


quil_out = circuit.out()
circuit = parse_program(quil_out) # new circuit


result = qc.run(executable).readout_data.get('ro')

counts = get_qiskit_like_output(result)
RESULT = counts


if __name__ == '__main__':
    from utils import display_results
    display_results( {"result": RESULT })

