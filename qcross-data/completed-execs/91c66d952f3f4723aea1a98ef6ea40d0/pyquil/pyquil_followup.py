...
defns = get_custom_get_definitions("CU1Gate", "U2Gate", "CCXGate", "RZGate", "CRZGate", "ZGate", "SdgGate", "XGate", "CHGate", "CSXGate", "RCCXGate")

circuit += defns

circuit.inst(Gates.RZGate(6.163759533339787, 9 ))
circuit.inst(Gates.CRZGate(4.2641612072511235, 2, 6 ))
circuit.inst(Gates.ZGate( 1 ))
circuit.inst(Gates.CCXGate( 5, 4, 3 ))
circuit.inst(Gates.ZGate( 6 ))
circuit.inst(Gates.XGate( 3 ))
circuit.inst(Gates.RCCXGate( 0, 2, 8 ))
circuit.inst(Gates.RZGate(4.229610589867865, 10 ))
circuit.inst(Gates.CCXGate( 3, 0, 6 ))
circuit.inst(Gates.SdgGate( 3 ))
circuit.inst(Gates.U2Gate(4.214504315296764, 4.6235667602042065)( 0 ))
circuit.inst(Gates.CSXGate( 9, 6 ))
circuit.inst(Gates.CHGate( 10, 3 ))
circuit.inst(Gates.CU1Gate(4.028174522740928, 4, 10 ))

qr_edd40d = circuit.declare("qr_edd40d", "BIT", 7)
