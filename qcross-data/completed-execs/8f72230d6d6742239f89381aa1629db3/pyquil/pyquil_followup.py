...
defns = get_custom_get_definitions("CSXGate", "SGate", "CRXGate", "CRZGate", "U2Gate", "C3SXGate", "ECRGate", "RZGate", "iSwapGate", "ZGate", "CU1Gate", "CHGate", "XGate", "SdgGate", "RCCXGate", "CPhaseGate")
...
circuit.inst(Gates.CRXGate(2.0099472182748075, 0, 3 ))
circuit.inst(Gates.C3SXGate( 5, 4, 2, 0 ))
circuit.inst(Gates.CHGate( 1, 5 ))
circuit.inst(Gates.C3SXGate( 4, 0, 2, 3 ))
circuit.inst(Gates.ZGate( 0 ))
circuit.inst(Gates.ECRGate( 5, 6 ))
circuit.inst(Gates.SdgGate( 5 ))
circuit.inst(Gates.RCCXGate( 0, 3, 4 ))
circuit.inst(Gates.SGate( 2 ))
circuit.inst(Gates.RZGate(4.229610589867865, 2 ))
circuit.inst(Gates.C3SXGate( 4, 0, 2, 6 ))
circuit.inst(Gates.CU1Gate(3.2142159669963557, 1, 4 ))
circuit.inst(Gates.CRXGate(5.94477504571567, 1, 5 ))
circuit.inst(Gates.CHGate( 1, 4 ))
circuit.inst(Gates.C3SXGate( 0, 4, 6, 1 ))
circuit.inst(Gates.CSXGate( 4, 0 ))

subcircuit = Program()
subcircuit.inst(Gates.iSwapGate( 2, 5 ))
subcircuit.inst(Gates.CPhaseGate(3.326780904591333, 4, 5 ))
subcircuit.inst(Gates.CU1Gate(2.0685963035149753, 1, 4 ))
circuit.inst(subcircuit)
circuit.inst(subcircuit.dagger())

circuit.inst(Gates.ZGate( 3 ))
circuit.inst(Gates.CRZGate(4.833923139882297, 4, 5 ))
circuit.inst(Gates.CU1Gate(4.028174522740928, 2, 1 ))
circuit.inst(Gates.C3SXGate( 0, 4, 3, 1 ))
circuit.inst(Gates.CRZGate(2.586208953975239, 5, 0 ))
