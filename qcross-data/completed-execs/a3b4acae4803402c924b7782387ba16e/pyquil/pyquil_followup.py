...
defns = get_custom_get_definitions("CCXGate", "UGate", "RCCXGate", "RZGate", "TGate", "CUGate", "CRXGate", "SGate", "ZGate", "C3SXGate", "CRZGate", "CU1Gate", "SXGate", "RZZGate")

circuit += defns

circuit.inst(Gates.RZGate(6.163759533339787, 5 ))
circuit.inst(Gates.ZGate( 0 ))
circuit.inst(Gates.CRZGate(4.2641612072511235, 4, 5 ))
circuit.inst(Gates.CUGate(0.5112149185250571, 5.897054719225356, 2.3864521352475245, 5.987304452123941, 4, 0 ))
circuit.inst(Gates.C3SXGate( 3, 5, 2, 4 ))
circuit.inst(Gates.CCXGate( 3, 1, 2 ))
circuit.inst(Gates.C3SXGate( 5, 0, 1, 2 ))
circuit.inst(Gates.ZGate( 2 ))
circuit.inst(Gates.ZGate( 4 ))
circuit.inst(Gates.TGate( 5 ))
circuit.inst(Gates.RCCXGate( 1, 0, 5 ))
circuit.inst(Gates.SGate( 1 ))
circuit.inst(Gates.CRZGate(4.167661441102218, 3, 1 ))
circuit.inst(Gates.RZGate(4.229610589867865, 3 ))
circuit.inst(Gates.C3SXGate( 2, 4, 3, 0 ))
...
circuit.inst(Gates.RZZGate(5.1829934776392745)( 2, 1 ))
circuit.inst(Gates.SGate( 5 ))
circuit.inst(Gates.SXGate( 2 ))
circuit.inst(Gates.ZGate( 5 ))
circuit.inst(Gates.CRZGate(4.833923139882297, 2, 1 ))
circuit.inst(Gates.CU1Gate(4.028174522740928, 3, 5 ))
circuit.inst(Gates.C3SXGate( 0, 2, 5, 4 ))
circuit.inst(Gates.CRZGate(2.586208953975239, 4, 1 ))
circuit.inst(Gates.CRXGate(2.6687018103754414, 5, 1 ))
