
import cirq

from bloqs.ext.cirq import Gates
from bloqs.ext.cirq.utils import get_qiskit_like_output
from sympy import Symbol
from cirq.contrib.qasm_import import circuit_from_qasm
import numpy as np


qr = [cirq.NamedQubit('q' + str(i)) for i in range(5)]

p_d5e26f = Symbol('p_d5e26f')
p_90ec67 = Symbol('p_90ec67')
p_263cbf = Symbol('p_263cbf')
p_3818c9 = Symbol('p_3818c9')
p_5671c2 = Symbol('p_5671c2')
p_3be06b = Symbol('p_3be06b')
p_1d1ce1 = Symbol('p_1d1ce1')

circuit = cirq.Circuit()

circuit.append(Gates.RZGate(p_d5e26f)( qr[1] ))
circuit.append(Gates.SXdgGate( qr[4] ))
circuit.append(Gates.CSXGate( qr[3], qr[0] ))
circuit.append(Gates.ECRGate( qr[1], qr[4] ))
circuit.append(Gates.CRXGate(2.0099472182748075)( qr[2], qr[1] ))
circuit.append(Gates.SGate( qr[0] ))
circuit.append(Gates.CRZGate(1.0296448789776642)( qr[1], qr[2] ))
circuit.append(Gates.CHGate( qr[3], qr[2] ))
circuit.append(Gates.RYYGate(p_3818c9)( qr[0], qr[4] ))
circuit.append(Gates.ZGate( qr[4] ))
circuit.append(Gates.TGate( qr[2] ))
circuit.append(Gates.CSXGate( qr[3], qr[2] ))
circuit.append(Gates.XGate( qr[0] ))
circuit.append(Gates.SGate( qr[2] ))
circuit.append(Gates.CUGate(p_263cbf, 4.167661441102218, 4.623446645668956, 3.865496458458116)( qr[3], qr[2] ))
circuit.append(Gates.RZGate(4.229610589867865)( qr[3] ))
circuit.append(Gates.RYYGate(p_90ec67)( qr[0], qr[4] ))
circuit.append(Gates.CU1Gate(p_1d1ce1)( qr[1], qr[0] ))
circuit.append(Gates.UGate(5.887184334931191, p_3be06b, p_5671c2)( qr[2] ))
circuit.append(Gates.CHGate( qr[4], qr[0] ))
circuit.append(Gates.ZGate( qr[0] ))
circuit.append(cirq.measure(qr[0], key='cr0'))
circuit.append(cirq.measure(qr[3], key='cr1'))
circuit.append(cirq.measure(qr[4], key='cr2'))
circuit.append(cirq.measure(qr[1], key='cr3'))
circuit.append(cirq.measure(qr[2], key='cr4'))



circuit = cirq.resolve_parameters(circuit, {
    "p_d5e26f": 6.163759533339787,
    "p_90ec67": 5.398622178940033,
    "p_263cbf": 5.708725119517347,
    "p_3818c9": 1.6723037552953224,
    "p_5671c2": 1.4112277317699358,
    "p_3be06b": 0.07157463504881167,
    "p_1d1ce1": 3.2142159669963557
}, recursive=True)
        










simulator = cirq.Simulator(seed=np.random.RandomState())


result = simulator.run(circuit, repetitions=979)
# result_dict = dict(result.multi_measurement_histogram()
# keys = list(map(lambda arr: reduce(lambda x, y: str(x) + str(y), arr[::-1]), result_dict.keys()))
counts = get_qiskit_like_output(result, keys=['cr0', 'cr1', 'cr2', 'cr3', 'cr4'])

RESULT = counts


if __name__ == '__main__':
    from qcross.utils import display_results
    display_results( {"result": RESULT })

