...
defns = get_custom_get_definitions("XGate", "RZGate", "CRXGate", "RYYGate", "CUGate", "U2Gate", "TGate", "SdgGate", "CSXGate", "ZGate", "C3SXGate", "CRZGate", "CU1Gate", "SXGate", "RZZGate")
...
circuit.inst(Gates.CRZGate(1.0296448789776642, 5, 1 ))
circuit.inst(Gates.C3SXGate( 2, 3, 1, 6 ))
...
circuit.inst(Gates.CU1Gate(3.2142159669963557, 7, 2 ))
circuit.inst(Gates.CRXGate(5.94477504571567, 1, 7 ))
circuit.inst(Gates.RZZGate(5.1829934776392745)( 3, 2 ))
circuit.inst(Gates.CSXGate( 2, 0 ))
circuit.inst(Gates.ZGate( 1 ))
circuit.inst(Gates.RZGate(3.775592041307464, 7 ))
circuit.inst(Gates.CRXGate(0.7279391018916035, 2, 6 ))
circuit.inst(Gates.CUGate(5.03147076606842, 5.0063780207098425, 3.1562533916051736, 4.940217775579305, 1, 0 ))
circuit.inst(Gates.U2Gate(2.5163050709890156, 2.1276323672732023)( 0 ))
circuit.inst(Gates.TGate( 1 ))
circuit.inst(Gates.SdgGate( 2 ))
circuit.inst(Gates.RZZGate(3.950837470808744)( 6, 7 ))
