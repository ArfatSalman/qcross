...
defns = get_custom_get_definitions("SdgGate", "CHGate", "CU1Gate", "CSXGate", "TGate", "SGate", "SXGate", "U2Gate", "CCXGate", "RCCXGate", "RZGate", "CRZGate", "ZGate", "XGate", "CUGate", "C3SXGate")

circuit += defns

circuit.inst(Gates.RZGate(6.163759533339787, 0 ))
circuit.inst(Gates.CSXGate( 1, 7 ))
circuit.inst(Gates.CUGate(0.5112149185250571, 5.897054719225356, 2.3864521352475245, 5.987304452123941, 8, 2 ))
circuit.inst(Gates.SdgGate( 3 ))
circuit.inst(Gates.C3SXGate( 8, 0, 4, 6 ))
circuit.inst(Gates.ZGate( 1 ))
circuit.inst(Gates.XGate( 6 ))
circuit.inst(Gates.SGate( 8 ))
circuit.inst(Gates.SGate( 0 ))
circuit.inst(Gates.C3SXGate( 3, 5, 1, 8 ))
circuit.inst(Gates.SXGate( 8 ))
circuit.inst(Gates.CU1Gate(3.2142159669963557, 0, 5 ))
circuit.inst(Gates.CRZGate(1.4112277317699358, 6, 0 ))
circuit.inst(Gates.ZGate( 1 ))
circuit.inst(Gates.CSXGate( 8, 1 ))
circuit.inst(Gates.ZGate( 4 ))
circuit.inst(Gates.CHGate( 2, 3 ))
circuit.inst(Gates.CSXGate( 5, 8 ))
circuit.inst(Gates.CRZGate(2.586208953975239, 3, 1 ))
...
circuit.inst(Gates.RCCXGate( 7, 8, 3 ))
circuit.inst(Gates.TGate( 8 ))

qr_1f657d = circuit.declare("qr_1f657d", "BIT", 2)
