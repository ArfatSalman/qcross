...
qc.append(CUGate(0.5112149185250571, 5.897054719225356, 2.3864521352475245,
    5.987304452123941), qargs=[qr[7], qr[2]], cargs=[])
qc.append(SdgGate(), qargs=[qr[6]], cargs=[])
qc.append(C3SXGate(), qargs=[qr[7], qr[3], qr[5], qr[8]], cargs=[])
...
qc.append(C3SXGate(), qargs=[qr[6], qr[0], qr[4], qr[7]], cargs=[])
...
backend_d9c002e6cca643eeb65b244bbc3d8a11 = Aer.get_backend('qasm_simulator')
counts = execute(qc, backend=backend_d9c002e6cca643eeb65b244bbc3d8a11, shots=3919).result().get_counts(qc)
RESULT = counts
