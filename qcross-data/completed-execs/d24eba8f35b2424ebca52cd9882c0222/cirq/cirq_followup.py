
import cirq

from bloqs.ext.cirq import Gates
from bloqs.ext.cirq.utils import get_qiskit_like_output
from sympy import Symbol
from cirq.contrib.qasm_import import circuit_from_qasm
import numpy as np


qr = [cirq.NamedQubit('q' + str(i)) for i in range(5)]

p_cd62bb = Symbol('p_cd62bb')
p_d57ef9 = Symbol('p_d57ef9')

circuit = cirq.Circuit()

circuit.append(Gates.RZGate(p_d57ef9)( qr[2] ))
circuit.append(Gates.SXdgGate( qr[3] ))
circuit.append(Gates.CSXGate( qr[0], qr[1] ))

subcircuit = cirq.Circuit()
subcircuit.append(Gates.DCXGate( qr[0], qr[4] ))
subcircuit.append(Gates.CPhaseGate(1.6161683469432118)( qr[0], qr[4] ))
subcircuit.append(Gates.RXGate(p_cd62bb)( qr[0] ))
subcircuit.append(Gates.ZGate( qr[1] ))
subcircuit.append(Gates.CXGate( qr[3], qr[2] ))
circuit.append(subcircuit)
circuit.append(cirq.inverse(cirq.expand_composite(subcircuit)))

circuit.append(Gates.ECRGate( qr[2], qr[3] ))
circuit.append(Gates.CRXGate(2.0099472182748075)( qr[4], qr[2] ))
circuit.append(cirq.measure(qr[1], key='cr0'))
circuit.append(cirq.measure(qr[0], key='cr1'))
circuit.append(cirq.measure(qr[3], key='cr2'))
circuit.append(cirq.measure(qr[2], key='cr3'))
circuit.append(cirq.measure(qr[4], key='cr4'))



circuit = cirq.resolve_parameters(circuit, {
    "p_cd62bb": 6.033961191253911,
    "p_d57ef9": 6.163759533339787
}, recursive=True)
        










simulator = cirq.Simulator(seed=np.random.RandomState())


result = simulator.run(circuit, repetitions=979)
# result_dict = dict(result.multi_measurement_histogram()
# keys = list(map(lambda arr: reduce(lambda x, y: str(x) + str(y), arr[::-1]), result_dict.keys()))
counts = get_qiskit_like_output(result, keys=['cr0', 'cr1', 'cr2', 'cr3', 'cr4'])

RESULT = counts


if __name__ == '__main__':
    from qcross.utils import display_results
    display_results( {"result": RESULT })

