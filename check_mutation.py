...
EXEC_METADATA_FOLDER = "data/mutation-check/"
...
DONE_FILE = open("MUT_DONE.csv", "r+", encoding="utf-8")
...
def execute_file(file):
    ...
    try:
        with open(src, encoding="ascii") as f:
            ...
            m, mutated_src = CirqCircuit(qiskit_src).get_follow_up(
                {"add_unitary": False, "seed": 1223, "qubit_mutation": 1}
            )
            try:
                mutated_res, _ = execute_single_py_program(mutated_src)
                metadata["mutated_res"] = mutated_res
            except Exception as error:
                exception = True
                metadata["src_Error"] = str(error)
                mutated_res = {}
                print(f"Error while executing mutated src of {file}")
                print(colored(error, "red"))
    ...
    global TOTAL
    TOTAL += 1
    if exception or src_res == mutated_res:
        global FAILURES
        FAILURES += 1
        print(colored("incorrect result", "red"))
        display_results({"src": src_res, "follow": mutated_res})
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        for file in os.listdir(SRC_FOLDER):
            ...
            count -= 1
            execute_file(file)
    print(colored(f'SUCCESS={SUCCESS_COUNT}, FAILURES={FAILURES}, TOTAL={TOTAL}', 'green'))
    DONE_FILE.close()
